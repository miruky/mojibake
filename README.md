...
候補は復元文をクリップボードへコピーでき、「共有リンク」で入力とモードをURLに符号化して渡せます(データは送信しません)。復号に失敗したバイトは復元文の中で印をつけて示します。右上で配色をライト / OS追従 / ダークに切り替えられ、入力内容とモードはブラウザに自動保存されます。キーボードでは <kbd>g</kbd> / <kbd>b</kbd> でモード切替、<kbd>e</kbd> で例の読み込み、<kbd>l</kbd> で共有リンクのコピーができます。
...
- `src/lib/share.ts` — 入力とモードのURL符号化
